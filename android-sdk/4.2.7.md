...
## 4.2.7 version<a id='4.2.7'></a>

## Table of Contents
* [What's New](#4.2.7-what-is-new)
* [Setup](#4.2.7-setup)
* [Usage](#4.2.7-usage)
* [Offline mode](#4.2.7-offline)
* [Settings сlass](#4.2.7-settings-class)
* [Functions](#4.2.7-functions)
* [Events](#4.2.7-events)
* [Festivals](#4.2.7-fest)
* [Navigation](#4.2.7-navigation)
* [GlobalLocationProvider](#4.2.7-global-location-provider)
* [CrowdConnected location provider](#4.2.7-cc-navigation)
* [IndoorAtlas location provider](#4.2.7-ia-navigation)
* [ExpoFP-GPS location provider](#4.2.7-efp-navigation)

## What's New in ExpoFP Fplan version 4.2.7<a id='4.2.7-what-is-new'></a>

In this version, the functionality responsible for caching has been improved.

## Setup<a id='4.2.7-setup'></a>
...
```java
dependencies {
    implementation 'com.expofp:common:4.2.7'
    implementation 'com.expofp:fplan:4.2.7'
    
    //If you want to use navigation from CrowdConnected, add a link to the package
    //implementation 'com.expofp:crowdconnected:4.2.7'
    
    //If you want to use navigation from IndoorAtlas, add a link to the package
    //implementation 'com.expofp:indooratlas:4.2.7'
...
## Offline mode<a id='4.2.7-offline'></a>
...
3.<a id='4.2.7-offline-init'></a> In the init() method, it is possible to specify the path to the offline archive with the plan. 
...
## Settings сlass<a id='4.2.7-settings-class'></a>
...
- loadingTimeout(int, optional) - timeout for loading the plan, after this time has elapsed, 
  FplanView will try to load the [offline version](#4.2.7-offline-init) or give an error. 
...
Specifies a subscriber for plan events. Events are described in the [Events](#4.2.7-events) chapter.
...
Specifies a subscriber for Festival events. Festival events are described in the [Festival events](#4.2.7-fest-events) chapter.
...
Specifies the location provider. The location provider is described in the [Navigation](#4.2.7-navigation) chapter.
...
Specifies that the GlobalLocationProvider should be used. The GlobalLocationProvider is described in the [GlobalLocationProvider](#4.2.7-global-location-provider) chapter.
...
## Functions<a id='4.2.7-functions'></a>
...
#### Festival events <a id='4.2.7-fest-events'></a>
...
## Navigation<a id='4.2.7-navigation'></a>
...
#### GlobalLocationProvider<a id='4.2.7-global-location-provider'></a>
...
## CrowdConnected location provider<a id='4.2.7-cc-navigation'></a>
...
When using CrowdConnectedProvider, it is recommended to run it in the background (when the program starts) using the [GlobalLocationProvider](#4.2.7-global-location-provider).
...
## IndoorAtlas location provider<a id='4.2.7-ia-navigation'></a>
...
## ExpoFP-GPS location provider<a id='4.2.7-efp-navigation'></a>
...
```java
dependencies {
    implementation 'com.expofp:gpsprovider:4.2.7'
    ... 
}
```
