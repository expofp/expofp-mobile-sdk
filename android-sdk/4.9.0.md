...
## 4.9.0 version<a id='4.9.0'></a>

## Table of Contents
* [What's New](#4.9.0-what-is-new)
* [Recommendations for use](#4.9.0-recommendations)
* [Setup](#4.9.0-setup)
* [Usage](#4.9.0-usage)
* [Offline mode](#4.9.0-offline)
* [Settings сlass](#4.9.0-settings-class)
* [Functions](#4.9.0-functions)
* [Events](#4.9.0-events)
* [Festivals](#4.9.0-fest)
* [Navigation](#4.9.0-navigation)
* [GlobalLocationProvider](#4.9.0-global-location-provider)
* [CrowdConnected location provider](#4.9.0-cc-navigation)
* [IndoorAtlas location provider](#4.9.0-ia-navigation)

## What's New in ExpoFP Fplan version 4.9.0<a id='4.9.0-what-is-new'></a>

The com.expofp.crowdconnected package has been split into 2 packages: com.expofp.crowdconnected and com.expofp.crowdconnectedbackground. The com.expofp.crowdconnected package works as usual and does not require special permissions to access the location in the background, this simplifies the process of publishing applications in the Google Play Market. If you need the background location update functionality, you need to use com.expofp.crowdconnectedbackground instead of com.expofp.crowdconnected.

## Recommendations for use<a id='4.9.0-recommendations'></a>

FplanView should be initialized only once, then it should be hidden and shown as needed.
Interaction with the FplanView should only occur using functions (selectBooth(), selectExhibitor(), etc.) and events (onFpReady, onBoothClick, etc.).

Example: <https://github.com/expofp/expofp-demo-android>

## Setup<a id='4.9.0-setup'></a>
...
```java
dependencies {
    implementation 'com.expofp:common:4.9.0'
    implementation 'com.expofp:fplan:4.9.0'
    
    //If you want to use navigation from CrowdConnected, add a link to the package
    //implementation 'com.expofp:crowdconnected:4.9.0'
    
    //If you want to use navigation from IndoorAtlas, add a link to the package
    //implementation 'com.expofp:indooratlas:4.9.0'
...
## Offline mode<a id='4.9.0-offline'></a>
...
## Settings сlass<a id='4.9.0-settings-class'></a>
...
- loadingTimeout(int, optional) - timeout for loading the plan, after this time has elapsed, 
  FplanView will try to load the [offline version](#4.9.0-offline-init) or give an error. 
...
Specifies a subscriber for plan events. Events are described in the [Events](#4.9.0-events) chapter.
...
Specifies a subscriber for Festival events. Festival events are described in the [Festival events](#4.9.0-fest-events) chapter.
...
Specifies the location provider. The location provider is described in the [Navigation](#4.9.0-navigation) chapter.
...
Specifies that the GlobalLocationProvider should be used. The GlobalLocationProvider is described in the [GlobalLocationProvider](#4.9.0-global-location-provider) chapter.
...
## Functions<a id='4.9.0-functions'></a>
...
#### Festival events <a id='4.9.0-fest-events'></a>
...
## Navigation<a id='4.9.0-navigation'></a>
...
#### GlobalLocationProvider<a id='4.9.0-global-location-provider'></a>
...
## CrowdConnected location provider<a id='4.9.0-cc-navigation'></a>
...
```java
dependencies {
    implementation 'com.expofp:crowdconnectedbackground:4.9.0'
    ... 
}
```
...
When using CrowdConnectedProvider, it is recommended to run it in the background (when the program starts) using the [GlobalLocationProvider](#4.9.0-global-location-provider).
...
## IndoorAtlas location provider<a id='4.9.0-ia-navigation'></a>
