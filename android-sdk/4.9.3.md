...
## 4.9.3 version<a id='4.9.3'></a>

## Table of Contents
* [What's New](#4.9.3-what-is-new)
* [Recommendations for use](#4.9.3-recommendations)
* [Setup](#4.9.3-setup)
* [Usage](#4.9.3-usage)
* [Offline mode](#4.9.3-offline)
* [Settings сlass](#4.9.3-settings-class)
* [Functions](#4.9.3-functions)
* [Events](#4.9.3-events)
* [Festivals](#4.9.3-fest)
* [Navigation](#4.9.3-navigation)
* [GlobalLocationProvider](#4.9.3-global-location-provider)
* [CrowdConnected location provider](#4.9.3-cc-navigation)
* [IndoorAtlas location provider](#4.9.3-ia-navigation)

## What's New in ExpoFP Fplan version 4.9.3<a id='4.9.3-what-is-new'></a>

The 'fplanView.init()' method has been renamed to 'fplanView.load()'.
The ability to work with plans in the background has been added (using the SharedFplanView control).

## Recommendations for use<a id='4.9.3-recommendations'></a>
...
[Important!] It is recommended to create a SharedFplanView instance each time you want to display a plan (and call the 'sharedFplanView.load()' method). 
...
To display the plan again, you must create a new SharedFplanView instance and call the 'sharedFplanView.load()' method.

## Setup<a id='4.9.3-setup'></a>
...
```java
dependencies {
    implementation 'com.expofp:common:4.9.3'
    implementation 'com.expofp:fplan:4.9.3'
    
    //If you want to use navigation from CrowdConnected, add a link to the package
    //implementation 'com.expofp:crowdconnected:4.9.3'
    
    //If you want to use navigation from IndoorAtlas, add a link to the package
    //implementation 'com.expofp:indooratlas:4.9.3'
...
## Offline mode<a id='4.9.3-offline'></a>
...
## Settings сlass<a id='4.9.3-settings-class'></a>
...
- loadingTimeout(int, optional) - timeout for loading the plan, after this time has elapsed, 
  FplanView will try to load the [offline version](#4.9.3-offline-init) or give an error. 
...
Specifies a subscriber for plan events. Events are described in the [Events](#4.9.3-events) chapter.
...
Specifies a subscriber for Festival events. Festival events are described in the [Festival events](#4.9.3-fest-events) chapter.
...
Specifies the location provider. The location provider is described in the [Navigation](#4.9.3-navigation) chapter.
...
Specifies that the GlobalLocationProvider should be used. The GlobalLocationProvider is described in the [GlobalLocationProvider](#4.9.3-global-location-provider) chapter.
...
## Functions<a id='4.9.3-functions'></a>
...
#### Festival events <a id='4.9.3-fest-events'></a>
...
## Navigation<a id='4.9.3-navigation'></a>
...
#### GlobalLocationProvider<a id='4.9.3-global-location-provider'></a>
...
## CrowdConnected location provider<a id='4.9.3-cc-navigation'></a>
...
```java
dependencies {
    implementation 'com.expofp:crowdconnectedbackground:4.9.3'
    ... 
}
```
...
When using CrowdConnectedProvider, it is recommended to run it in the background (when the program starts) using the [GlobalLocationProvider](#4.9.3-global-location-provider).
...
## IndoorAtlas location provider<a id='4.9.3-ia-navigation'></a>
