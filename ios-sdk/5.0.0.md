...
await presenter.applyNewAdditionalParams(newParams)
...
```swift
await presenter.removeAdditionalParams(forKeys: ["noOverlay"])
```
...
await presenter.applyNewLocationProvider(locationProvider)

or

await presenter.applyNewLocationProvider(nil)
```
...
await presenter.setMessageListener(messageListener)

or

await presenter.setMessageListener(nil)
```
...
Clear cache and reload a plan with all previously setted parameters and location settings<br>
...
> Plan will call `startUpdatingLocation()` when it appers and `stopUpdatingLocation()` when it disappears if locationProvider is not set as Global.
