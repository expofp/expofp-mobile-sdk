...
## 4.9.2 version<a id='4.9.2'></a>

## Table of Contents
* [What's New](#4.9.2-what-is-new)
* [Recommendations for use](#4.9.2-recommendations)
* [Installation](#4.9.2-installation)
* [UIKit](#4.9.2-uikit)
    * [Usage](#4.9.2-uikit-usage)
    * [Offline mode](#4.9.2-uikit-offline)
    * [Settings сlass](#4.9.2-uikit-settings-class)
    * [Functions](#4.9.2-uikit-functions)
    * [Events](#4.9.2-uikit-events)
    * [Festivals](#4.9.2-uikit-fest)
* [SwiftUI](#4.9.2-swiftui)
    * [Usage](#4.9.2-swiftui-usage)
    * [Offline mode](#4.9.2-swiftui-offline)
    * [Settings сlass](#4.9.2-swiftui-settings-class)
    * [Functions](#4.9.2-swiftui-functions)
    * [Events](#4.9.2-swiftui-events)
    * [Festivals](#4.9.2-swiftui-fest)
* [Navigation](#4.9.2-navigation)
* [GlobalLocationProvider](#4.9.2-global-location-provider)
* [CrowdConnected location provider](#4.9.2-cc-navigation)
* [IndoorAtlas location provider](#4.9.2-ia-navigation)

## What's New in ExpoFP Fplan version 4.9.2<a id='4.9.2-what-is-new'></a>

Functions for UIKit and SwiftUI:

* `highlightBooths(externalIds: [String])`
* `getFloors(completion: [ExpoFpFloor] -> Void)`
* `activateFloor(floor: ExpoFpFloor)`

Events:

* `setOnCurrentFloorChangedCallback(callback: ExpoFpFloor -> Void)` for UIKit
* `onCurrentFloorChangedCallback(callback: ExpoFpFloor -> Void)` for SwiftUI

## Recommendations for use<a id='4.9.2-recommendations'></a>
...
## Offline mode<a id='4.9.2-uikit-offline'></a>

### ExpoFpOfflinePlanInfoLoader

Designed to work with offline plans. [Important!] ExpoFpOfflinePlanInfoLoader only downloads offline plans, but it does not delete old versions of plans from the cache. This allows to open an old version of a plan while a new version of the plan is being downloaded to the cache. It is very important to delete old versions of plans if they are not used.

Save a plan:

```swift
let expoKey = "demo"
let result = await ExpoFpOfflinePlanInfoLoader.loadPlanToCacheDirectory(with: expoKey) // also available with completion

do {
	let offlinePlanInfo = result.get()
} catch {
	print(error)
}
```

##### OfflinePlanInfo  properties:

* `expoKey(String) - Expo key`
* `version(Int) - Version of offline plan`

Open saved plan:

```swift
let plan = UIFplanView()
plan.openFile(with: offlinePlanInfo)
```

Manage saved plans:

```swift
let offlinePlansInfo = await ExpoFpOfflinePlanInfoLoader.getLoadedPlansInfo() // also available with completion

ExpoFpOfflinePlanInfoLoader.removeLoadedPlan(with: offlinePlanInfo)
or
ExpoFpOfflinePlanInfoLoader.removeAllLoadedPlans()
```

## Settings сlass<a id='4.9.2-uikit-settings-class'></a>

The Settings class contains all the parameters necessary for the initialization and operation of the plan.

### Parameters passed to the constructor

- locationProvider(LocationProvider, optional) - [location provider](#4.9.2-navigation). The default value for this parameter is nil.

```swift
fplanUiView.load("https://demo.expofp.com/", settings: Settings(locationProvider: GpsProvider()))
```

- useGlobalLocationProvider(boolean) - a parameter indicating that the [global location provider](#4.9.2-global-location-provider) should be used.
...
## Functions<a id='4.9.2-uikit-functions'></a>
...
```swift
fplanUiView.highlightBooths(["720", "721"])
```
...
```swift
fplanUiView.selectRoute(waypoints: [RouteWaypoint])
```

##### com.expofp.fplan.models.RouteWaypoint constructors:

* `RouteWaypoint(value: String)`
* `RouteWaypoint(value: CurrentPosition)`
...
#### Get Floors:
```swift
fplanUiView.getFloors{floors in }
```

#### Activate Floor:
```swift
fplanUiView.activateFloor(floor)
```

#### Floor properties:
* `name(String) - floor name`
* `shortName(String) - floor short name`
* `description(String) - floor descriotion`
* `index(Int) - floor index`
* `isActive(Bool) - floor is currently active`
* `isDisabled(Bool) - floor is disabled for use`
...
## Events<a id='4.9.2-uikit-events'></a>
...
```swift
fplanUiView.setOnCurrentFloorChangedCallback { floor in
}
```
...
## Festivals<a id='4.9.2-uikit-fest'></a>
...
## Offline mode<a id='4.9.2-swiftui-offline'></a>
...
fplanView.openFile(with: offlinePlanInfo)
```
...
## Settings сlass<a id='4.9.2-swiftui-settings-class'></a>
...
## Functions<a id='4.9.2-swiftui-functions'></a>
...
```swift
fplanView.highlightBooths(["720", "721"])
```
...
```swift
fplanView.selectRoute(waypoints: [RouteWaypoint])
```
...
#### Get Floors:
```swift
fplanView.getFloors{floors in }
```

#### Activate Floor:
```swift
fplanView.activateFloor(floor)
```
...
## Events<a id='4.9.2-swiftui-events'></a>
...
struct FplanApp: App {
    ...
    var body: some Scene {
        WindowGroup {
            VStack
            {
                fplanView.onCurrentFloorChangedCallback { floor in
                }
                .onAppear {
                    fplanView.load("https://demo.expofp.com")
                }
...
## Festivals<a id='4.9.2-swiftui-fest'></a>
...
## Navigation<a id='4.9.2-navigation'></a>
...
## GlobalLocationProvider<a id='4.9.2-global-location-provider'></a>
...
## CrowdConnected location provider<a id='4.9.2-cc-navigation'></a>
...
```swift
pod 'ExpoFpCrowdConnected', '4.9.2'
```
...
When using CrowdConnectedProvider, it is recommended to run it in the background (when the program starts) using the [GlobalLocationProvider](#4.9.2-global-location-provider).
...
## IndoorAtlas location provider<a id='4.9.2-ia-navigation'></a>
