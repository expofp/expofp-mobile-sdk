...
## 4.8.14 version<a id='4.8.14'></a>

## Table of Contents
* [What's New](#4.8.14-what-is-new)
* [Recommendations for use](#4.8.14-recommendations)
* [Installation](#4.8.14-installation)
* [UIKit](#4.8.14-uikit)
    * [Usage](#4.8.14-uikit-usage)
    * [Offline mode](#4.8.14-uikit-offline)
    * [Settings сlass](#4.8.14-uikit-settings-class)
    * [Functions](#4.8.14-uikit-functions)
    * [Events](#4.8.14-uikit-events)
    * [Festivals](#4.8.14-uikit-fest)
* [SwiftUI](#4.8.14-swiftui)
    * [Usage](#4.8.14-swiftui-usage)
    * [Offline mode](#4.8.14-swiftui-offline)
    * [Settings сlass](#4.8.14-swiftui-settings-class)
    * [Functions](#4.8.14-swiftui-functions)
    * [Events](#4.8.14-swiftui-events)
    * [Festivals](#4.8.14-swiftui-fest)
* [Navigation](#4.8.14-navigation)
* [GlobalLocationProvider](#4.8.14-global-location-provider)
* [CrowdConnected location provider](#4.8.14-cc-navigation)
* [IndoorAtlas location provider](#4.8.14-ia-navigation)

## What's New in ExpoFP Fplan version 4.8.14<a id='4.8.14-what-is-new'></a>

SharedFplanView(SwiftUI) and SharedUIFplanView(UIKit) has been added.

## Recommendations for use<a id='4.8.14-recommendations'></a>
...
[Important!] It is recommended to create a SharedFplanView(SharedUIFplanView.preload for UIKit) instance each time you want to display a plan (and call the ‘sharedFplanView.load()’ method). 
After finishing working with the plan, you must call the ‘sharedFplanView.destroy()’ method. 
After calling the ‘sharedFplanView.destroy()’ method, the plan will continue to work in the background. 
To display the plan again, you must create a new SharedFplanView(SharedUIFplanView.preload) instance and call the ‘sharedFplanView.load()’ method.

## Installation<a id='4.8.14-installation'></a>

Cocoapods:

```swift
pod 'ExpoFpFplan', '4.8.14'
```

SPM: <https://github.com/expofp/expofp-fplan-ios-spm>

# UIKit<a id='4.8.14-uikit'></a>

This documentation block describes all the features and functions of ExpoFP SDK for the UIKit platform.

## Usage<a id='4.8.14-uikit-usage'></a>
...
## Offline mode<a id='4.8.14-uikit-offline'></a>
...
## Settings сlass<a id='4.8.14-uikit-settings-class'></a>

The Settings class contains all the parameters necessary for the initialization and operation of the plan.

### Parameters passed to the constructor

- locationProvider(LocationProvider, optional) - [location provider](#4.8.14-navigation). The default value for this parameter is nil.

```swift
fplanUiView.load("https://demo.expofp.com/", settings: Settings(locationProvider: GpsProvider()))
```

- useGlobalLocationProvider(boolean) - a parameter indicating that the [global location provider](#4.8.14-global-location-provider) should be used.
...
## Functions<a id='4.8.14-uikit-functions'></a>
...
## Events<a id='4.8.14-uikit-events'></a>
...
## Festivals<a id='4.8.14-uikit-fest'></a>
...
## Offline mode<a id='4.8.14-swiftui-offline'></a>
...
## Settings сlass<a id='4.8.14-swiftui-settings-class'></a>
...
## Functions<a id='4.8.14-swiftui-functions'></a>
...
## Events<a id='4.8.14-swiftui-events'></a>
...
## Festivals<a id='4.8.14-swiftui-fest'></a>
...
## Navigation<a id='4.8.14-navigation'></a>
...
## GlobalLocationProvider<a id='4.8.14-global-location-provider'></a>
...
## CrowdConnected location provider<a id='4.8.14-cc-navigation'></a>
...
```swift
pod 'ExpoFpCrowdConnected', '4.8.14'
```
...
When using CrowdConnectedProvider, it is recommended to run it in the background (when the program starts) using the [GlobalLocationProvider](#4.8.14-global-location-provider).
...
## IndoorAtlas location provider<a id='4.8.14-ia-navigation'></a>
